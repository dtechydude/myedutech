...
class ScoreEntryForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Pop the dynamic display labels passed from the view. Defaults match
        # the original static names, so nothing changes for a school that
        # never customizes SchoolYearSettings.
        self.ca1_label = kwargs.pop('ca1_label', 'CA1')
        self.ca2_label = kwargs.pop('ca2_label', 'CA2')
        self.ca3_label = kwargs.pop('ca3_label', 'CA3')
        self.exam_label = kwargs.pop('exam_label', 'Exam')

        super().__init__(*args, **kwargs)

        # Update CA fields with dynamic placeholders, max attributes, and labels
        ca_labels = {'ca1': self.ca1_label, 'ca2': self.ca2_label, 'ca3': self.ca3_label}
        for field_name in ['ca1', 'ca2', 'ca3']:
            self.fields[field_name].label = ca_labels[field_name]
            self.fields[field_name].widget = forms.NumberInput(attrs={
                'placeholder': f'Max Total: {self.max_ca}',
                'min': 0,
                'max': self.max_ca,
        ...
        # Update Exam field with dynamic placeholders, max attributes, and label
        self.fields['exam_score'].label = self.exam_label
        self.fields['exam_score'].widget = forms.NumberInput(attrs={
            'placeholder': f'Max: {self.max_exam}',
            'min': 0,
            'max': self.max_exam,
