...
# NEW LIST VIEW PREVENTING OTHER TEACHERS FROM SEEIN EVERY RESULTS
# class ReportCardListView(LoginRequiredMixin, TeacherRequiredMixin, View):
#     template_name = 'results/termly_report_card_list.html'
...
#                     # Filter: Only students in classes where this teacher is the form_teacher
#                     queryset = queryset.filter(current_class__form_teacher=teacher_profile)
                    
#                     # Optional: If they try to filter for a class that isn't theirs, force it back
#                     if selected_standard and selected_standard.form_teacher != teacher_profile:
#                         selected_standard = None 
#                         messages.warning(request, "You are only authorized to view your own class.")

#                 # Continue with your existing filtering logic
#                 if selected_standard:
#                     students = queryset.filter(current_class=selected_standard).order_by('last_name', 'first_name')
#                 else:
#                     # Only show students with scores in the allowed queryset
#                     students = queryset.filter(scores__term=selected_term).distinct().order_by('last_name', 'first_name')
                    
#                     if is_admin:
#                         messages.info(request, f"Displaying all students with scores for {selected_term.name}.")
#             else:
#                 messages.warning(request, "Please select a term to view students.")
...
#                 # Base Queryset for initial load
#                 queryset = Student.objects.filter(scores__term=current_term).distinct()
                
#                 # Apply Teacher Restriction on initial load
#                 if not is_admin and teacher_profile:
#                     students = queryset.filter(current_class__form_teacher=teacher_profile).order_by('last_name', 'first_name')
#                     messages.info(request, f"Showing students in your class for {current_term.name}.")
#                 else:
#                     students = queryset.order_by('last_name', 'first_name')
#                     messages.info(request, f"Showing students for the current term: {current_term.name}.")
...
#             'selected_term': selected_term,
#             'selected_standard': selected_standard,
#             'is_admin': is_admin, # Useful for template toggles
#         }
#         return render(request, self.template_name, context)
...
# #SESSION OVERAL RESULTS
# # used until 24/12/2025
