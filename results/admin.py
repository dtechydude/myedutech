...
@admin.register(SchoolYearSettings)
class SchoolYearSettingsAdmin(admin.ModelAdmin):
    list_display = ('__str__', 'max_ca_total', 'max_exam_score', 'ca1_label', 'ca2_label', 'ca3_label', 'exam_label', 'is_active')
    list_editable = ('is_active',)
    ...
    fieldsets = (
        ...
        ('Display Labels (optional)', {
            'fields': ('ca1_label', 'ca2_label', 'ca3_label', 'exam_label'),
            'description': "Leave blank to use the defaults (CA1, CA2, CA3, Exam). Renaming these only changes what shows on the score entry screen — it does not affect how scores are stored or validated."
        }),
        ('Status', {
            'fields': ('is_active',),
        }),
